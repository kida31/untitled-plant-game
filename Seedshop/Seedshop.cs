...
public partial class Seedshop : Control
{
    ...
    {
        { "Carrot", new Dictionary<string, int> { {"price", 10}, {"available", 5}, {"owned", 0} }},
        { "Tomato", new Dictionary<string, int> { {"price", 15}, {"available", 3}, {"owned", 0} }},
        { "Pumpkin", new Dictionary<string, int> { {"price", 20}, {"available", 2}, {"owned", 0} }}
    };
    ...
    {
        GetParent().Call("close_shop");  // Calls the close_shop function from the parent scene
    }

    private void OnSeedButtonPressed(string seedType)
    {
        GD.Print("Buying a " + seedType + " seed.");
        ...
        if (seedInfo["available"] > 0 && playerCurrency >= seedInfo["price"])
        {
            ...
            UpdateCurrencyLabel();
            GD.Print("Bought a " + seedType + " seed. Total Owned: " + seedInfo["owned"]);
        }
        else
        {
            GD.Print("Not enough seeds available or not enough currency!");
        }
    }
